...
<li>
    <div class="flex items-center">
        ...
        <a href="{{ route('admin.pengajuan.index') }}" class="text-sm font-medium text-gray-700 hover:text-blue-600">Kelola Pengajuan</a>
    </div>
</li>
<li>
    <div class="flex items-center">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-900 mt-1">{{ $status['text'] }}</p>
                            </div>
                    ...
                    <div class="p-6">
                        ...
                        @if($additionalData && !empty($additionalData))
                            <div class="mt-4 pt-4 border-t border-gray-200">
                                ...
                                {{-- Nested data (Orang Tua, Dosen Wali) --}}
                                                        ...
                                                        @if(is_array($subValue))
                                                            ...
                                                            <div class="mb-2 p-2 bg-white border rounded">
                                                                @foreach($item as $k => $v)
                                                                    <div class="grid grid-cols-2 gap-3">
                                                                        <div>
                                                                            <label class="text-xs font-medium text-gray-500">{{ ucfirst(str_replace('_', ' ', $k)) }}</label>
                                                                        </div>
                                                                        <div>
                                                                            <p class="text-sm text-gray-900 mt-1">{{ is_array($v) ? implode(', ', $v) : $v }}</p>
                                                                        </div>
                                                                    </div>
                                                                @endforeach
                                                            </div>
                                        ...
                                        <div class="relative flex space-x-3">
                                            <div>
                                                <span class="h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white
                                                    {{ $history->action == 'approved' ? 'bg-green-500' : ($history->action == 'rejected' ? 'bg-red-500' : 'bg-blue-500') }}">
                                                    <i class="fas {{ $history->action == 'approved' ? 'fa-check' : ($history->action == 'rejected' ? 'fa-times' : 'fa-clock') }} text-white text-xs"></i>
                                                </span>
                                            </div>
                                            <div class="flex-1 min-w-0">
                                                <div>
                                                    <p class="text-sm font-medium text-gray-900">
                                                        {{ ucfirst($history->action) }} oleh {{ $history->performedBy->name ?? 'System' }}
                                                    </p>
                ...
                <div class="bg-white shadow-sm rounded-xl overflow-hidden">
                    ...
                    <div class="p-4 space-y-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium text-gray-900">{{ $status['text'] }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Dibuat:</span>
                            <span class="font-medium text-gray-900">{{ $pengajuan->created_at->diffForHumans() }}</span>
                        </div>
                        @if($pengajuan->updated_at != $pengajuan->created_at)
                        <div class="flex justify-between">
                            <span class="text-gray-600">Update Terakhir:</span>
                            <span class="font-medium text-gray-900">{{ $pengajuan->updated_at->diffForHumans() }}</span>
                        </div>
                        @endif
                    </div>
                </div>
...
<div id="changeStatusModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-lg bg-white">
        <div class="mb-4">
            ...
            <p class="text-sm text-gray-500 mt-1">Status saat ini: <span class="font-medium">{{ $status['text'] }}</span></p>
        </div>
...
function submitChangeStatus(event) {
    ...
    fetch('/admin/pengajuan/{{ $pengajuan->id }}/change-status', {
        method: 'POST',
        headers: {
