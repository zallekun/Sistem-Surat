    ...
    <div class="max-w-full mx-auto">
        <div class="bg-white/95 backdrop-blur-sm shadow-sm rounded-xl overflow-hidden">
            ...
            <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                <form method="GET" action="{{ route('admin.pengajuan.index') }}" class="space-y-4">
                    ...
                    <div class="flex justify-between items-center">
                        <div class="flex gap-4 items-center">
                            ...
                            <a href="{{ route('admin.pengajuan.index') }}" class="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 text-sm font-medium transition">
                                <i class="fas fa-redo mr-1"></i>Reset
                            </a>
                            
                            <div class="flex gap-3 ml-4">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="show_stuck" value="1" {{ request('show_stuck') ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                    <span class="ml-2 text-sm text-gray-700">Stuck > 3 hari</span>
                                </label>
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="show_deleted" value="1" {{ request('show_deleted') ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                    ...
                    <!-- Applied Filters -->
                    @if(request()->hasAny(['search', 'prodi_id', 'status', 'jenis_surat_id', 'date_from']))
                                ...
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    @if(request('search'))
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                            "{{ request('search') }}"
                                            <a href="{{ route('admin.pengajuan.index', request()->except('search')) }}" class="ml-1.5 hover:text-blue-900">×</a>
                                        </span>
                                    @endif
                                    ...
                                    @if(request('prodi_id'))
                                        ...
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                            ...
                                            <a href="{{ route('admin.pengajuan.index', request()->except('prodi_id')) }}" class="ml-1.5 hover:text-green-900">×</a>
                                        </span>
                                    @endif
                                    ...
                                    @if(request('status'))
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                            ...
                                            <a href="{{ route('admin.pengajuan.index', request()->except('status')) }}" class="ml-1.5 hover:text-purple-900">×</a>
                                        </span>
                                    @endif
                                </div>
                                
                                <a href="{{ route('admin.pengajuan.index') }}" class="text-sm text-blue-600 hover:text-blue-800 font-medium">
                                    Hapus semua
                                </a>
                            </div>
                        </div>
                            ...
                            <table class="min-w-full">
                                <tbody class="bg-white divide-y divide-gray-100">
                                    @foreach($pengajuans as $index => $pengajuan)
                                        <tr class="hover:bg-blue-50 {{ $pengajuan->trashed() ? 'bg-red-50' : '' }}">
                                            ...
                                            <td class="px-4 py-4 w-36">
                                                ...
                                                @else
                                                    ...
                                                    <span class="inline-flex px-2.5 py-1 text-xs font-medium rounded-full {{ $statusClass }}">
                                                        {{ $statuses[$pengajuan->status] ?? ucfirst($pengajuan->status) }}
                                                    </span>
                                                @endif
                                            </td>
                                            <td class="px-4 py-4 w-32">
            ...
            @else
                ...
                <div class="text-center py-16">
                    ...
                    <p class="text-gray-500 text-sm">
                        @if(request()->hasAny(['search', 'prodi_id', 'status', 'jenis_surat_id']))
                            Coba ubah filter atau <a href="{{ route('admin.pengajuan.index') }}" class="text-blue-600 hover:text-blue-800 font-medium">reset pencarian</a>
                        @else
