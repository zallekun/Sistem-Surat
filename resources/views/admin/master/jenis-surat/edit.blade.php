{{-- resources/views/admin/master/jenis-surat/edit.blade.php --}}
@extends('layouts.app')
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white shadow-sm rounded-xl overflow-hidden">
            <div class="px-6 py-4 border-b bg-gradient-to-r from-purple-500 to-purple-600">
                <h2 class="text-xl font-bold text-white">Edit Jenis Surat</h2>
            </div>

            <form action="{{ route('admin.jenis-surat.update', $jenisSurat->id) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" 
                                  rows="4"
                                  class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $jenisSurat->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
