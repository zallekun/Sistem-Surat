...
class PengajuanSurat extends Model
{
    ...
    // Tambahkan accessor untuk clean additional_data
public function getAdditionalDataAttribute($value)
{
    ...
    // Jika string, clean dan decode
    if (is_string($value)) {
        // Remove weird double quotes at start and end
        $value = trim($value, '"');
...
// Tambahkan mutator untuk set additional_data dengan benar
...
{
    return $this->hasOne(\App\Models\SuratGenerated::class, 'pengajuan_id', 'id');
}
...
public function hasPdfFile(): bool
{
    return $this->status === 'completed' 
           && $this->suratGenerated 
           && !empty($this->suratGenerated->file_path)
           && \Illuminate\Support\Facades\Storage::disk('public')->exists($this->suratGenerated->file_path);
}
