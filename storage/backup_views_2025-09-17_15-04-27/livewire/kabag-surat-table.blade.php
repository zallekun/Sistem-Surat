<div>
                    ...
                    <tr class="odd:bg-white even:bg-gray-50 hover:bg-gray-100">
                        ...
                        <td class="px-6 py-4 text-center text-sm font-medium whitespace-nowrap">
                            ...
                            @if($surat->currentStatus?->kode_status !== 'arsip')
                                @if(Auth::user()->jabatan?->nama_jabatan === 'Kepala Bagian TU' && ($surat->currentStatus?->kode_status === 'disposisi_kabag' || $surat->currentStatus?->kode_status === 'ditolak_divisi'))
                                    <button wire:click="$dispatch('openDistribusiModal', { suratId: {{ $surat->id }} })" class="text-indigo-600 hover:text-indigo-900 ml-2 px-2 py-1 rounded-md" title="Distribusi Surat"><i class="fa-solid fa-share-nodes"></i> Distribusi</button>
                                @endif
                            @endif
                        </td>
                    </tr>
    ...
    <!-- Distribusi Modal -->
    <div x-data="{ open: false, currentSuratId: null, instruksi: '', tujuanDivisiId: '' }" x-show="open" @open-distribusi-modal.window="open = true; currentSuratId = $event.detail.suratId" @close-distribusi-modal.window="open = false; currentSuratId = null; instruksi = ''; tujuanDivisiId = ''" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full" style="display: none;">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <h3 class="text-lg font-medium leading-6 text-gray-900 mb-4">Distribusi Surat</h3>
            <div class="mt-2">
            ...
            <div class="mt-4">
                <label for="tujuan_divisi" class="block text-sm font-medium text-gray-700">Tujuan Divisi</label>
                <select x-model="tujuanDivisiId" id="tujuan_divisi" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    <option value="">Pilih Divisi</option>
                    @foreach($tujuanDivisis as $jabatan)
                        <option value="{{ $jabatan->id }}">{{ $jabatan->nama_jabatan }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mt-5 sm:mt-6">
                <button @click="$wire.distributeSurat(currentSuratId, instruksi, tujuanDivisiId); open = false;" type="button" class="inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:text-sm">
                    Submit Distribusi
                </button>
